...
def start_experiment(chunk_size, repetitions_num, aws_access_key_id, aws_secret_access_key):
    class_path = '.:/usr/local/share/java/org.hyperdex.client-1.8.1.jar:lib/aws-sdk/lib/aws-java-sdk-1.11.255.jar:lib/aws-sdk/third-party/lib/*'
    command = 'cd ~/solitude && javac -classpath "{}" $(find . -name \'*.java\')'.format(class_path)
    run(command)

    command = 'cd ~/solitude && java -classpath "{}" -Djava.library.path=/usr/local/lib ch/michel/test/AmazonS3 {} {} {} {} {}'.format(class_path, chunk_size, repetitions_num, COORDINATOR_IP, aws_access_key_id, aws_secret_access_key)
    run(command)
