 ...
 * 
 * Running:
 * java -classpath ".:lib/aws-sdk/lib/aws-java-sdk-1.11.255.jar:lib/aws-sdk/third-party/lib/*:lib/treedb.client-0.0.1-SNAPSHOT-jar-with-dependencies.jar" --add-modules=java.xml.bind,java.activation ch/michel/test/TreeDBPerformance 1000 1 AKIAI6DALQXSPRIEAPWQ zerBOTixXhgqvJuij00evoJOOHYDAuYR9cYTZXYy 127.0.0.1 8001
 * 
 * Compiling:
 ...
 */
public class TreeDBPerformance {
         ...
         */
         for (int i=0; i < experimentReps; i++) {
            ...
            for (int j=0; j < 4; j++) {
                ...
                switch (j) {
                    case 0: // Paillier 
                        streamID = trDB.createStream(k, "{ 'sum': true }", keys.publicKey, "S3");
                        break;
                    case 1: // EC ElGamal
                        streamID = trDB.createStream(k, "{ 'sum': true, 'algorithms': { 'sum': 'ecelgamal' } }", null, "S3");
                        break;
                    case 2: // OPE
                        streamID = trDB.createStream(k, "{ 'max': true }", null, "S3");
                        break;
                    case 3: // ORE
                        streamID = trDB.createStream(k, "{ 'max': true, 'algorithms': { 'max': 'ore' } }", null, "S3");
                        break;
                }
                ...
                for (Chunk c : chunks) {
                    ...
                    trDB.insert(streamID, c.getPrimaryAttribute(), data, metadata);
                }

                // Clean state of the experiment
                trDB.delete(streamID);
