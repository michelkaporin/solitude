...
import ch.michel.test.treedb.BaselineMetadata;
import ch.michel.test.treedb.TreeDBBaselineClient;
...
public class TreeDBBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        TreeDBBaselineClient trdbBaseline = null;
        if (s3Baseline) {
            cleanState(chunks, s3, BASELINE_S3_BUCKET, false);        
        } else {
            // Create connection to TreeDB Baseline server
            trdbBaseline = new TreeDBBaselineClient(trdbBaselineIP, trdbBaselinePort);
        ...
        // Populate S3/TreeDB-Baseline and TreeDB
        for (Chunk c : chunks) {
        ...
        List<TreeDBBenchmarkStats> baselineSumStats = new ArrayList<TreeDBBenchmarkStats>();
        List<TreeDBBenchmarkStats> baselineMaxStats = new ArrayList<TreeDBBenchmarkStats>();
        ...
        for (int i=0; i < experimentReps; i++) {
            ...
            if (s3Baseline) {
                for (int j = 0; j < chunks.size(); j++) {
                    ...
                    // Populate stats 
                    baselineSumStats.add(new TreeDBBenchmarkStats("S3", s3RetrievalTime, s3DecodeDecryptTime, s3SumComputationTime));
                    baselineMaxStats.add(new TreeDBBenchmarkStats("S3", s3RetrievalTime, s3DecodeDecryptTime, s3MaxComputationTime));
                }
            } else {
                // TreeDB Baseline
                for (int j = 0; j < chunks.size(); j++) {
                    ...
                    baselineSumStats.add(new TreeDBBenchmarkStats("TreeDB Baseline", retrievalTime, decodeDecryptTime, sumComputationTime));
                    baselineMaxStats.add(new TreeDBBenchmarkStats("TreeDB Baseline", retrievalTime, decodeDecryptTime, maxComputationTime));
                }
            }
