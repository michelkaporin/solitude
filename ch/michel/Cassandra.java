...
public class Cassandra implements Storage {
	...
	public boolean put(Chunk chunk, String table, byte[] data, boolean singleEntry) {
		ByteBuffer buffer = ByteBuffer.wrap(data);

		Date time = new Date(Long.valueOf(chunk.getPrimaryAttribute()));

		String insertStm = singleEntry
				? String.format("INSERT INTO %s.%s (key, value, tempSkin) VALUES (?,?,?)", KEYSPACE, table)
				: String.format("INSERT INTO %s.%s (key, value) VALUES (?,?)", KEYSPACE, table);
		PreparedStatement ps = session.prepare(insertStm);
		BoundStatement boundStatement = new BoundStatement(ps);
		Statement stm = singleEntry ? boundStatement.bind(time, buffer, chunk.secondAttribute)
				: boundStatement.bind(time, buffer);
	...
	public List<byte[]> getRange(int temp1, int temp2, String table) {
		...
		String stm = String.format("SELECT * FROM %s.%s WHERE tempSkin >= %s AND tempSkin <= %s", KEYSPACE, table, temp1, temp2);
	...
	public void createKeyspace(int replicationFactor) {
		StringBuilder sb = new StringBuilder("CREATE KEYSPACE IF NOT EXISTS ").append(KEYSPACE)
				.append(" WITH replication = {").append("'class':'").append("SimpleStrategy") // Replication strategy class
	...
	public void createSingleEntryTable(String tableName) {
		try {
			String stm = String.format(
					"CREATE TABLE IF NOT EXISTS %s.%s (key timestamp, value blob, tempSkin int, PRIMARY KEY (timestamp, tempSkin));", KEYSPACE,
					tableName);
			session.execute(stm);
